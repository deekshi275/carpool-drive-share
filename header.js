...
class Header extends HTMLElement {
    ...
    async loadNotificationCount() {
        try {
            ...
            // Check if there's a rating notification flag
            const ratingNotification = localStorage.getItem('ratingNotification');
            console.log('Rating notification flag:', ratingNotification);
            console.log('Total pending requests:', totalPendingRequests);
            if (totalPendingRequests > 0 || ratingNotification) {
                if (badge) {
                    badge.textContent = totalPendingRequests > 0 ? totalPendingRequests : '1';
                    badge.classList.add('show');
                    console.log('Showing notification badge with text:', badge.textContent);
                }
            } else {
                if (badge) {
